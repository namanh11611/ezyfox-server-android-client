...
public class MsgPackSimpleDeserializer
    ...
    private final MsgPackTypeParser typeParser
        = new MsgPackTypeParser();
    private final MapSizeDeserializer mapSizeDeserializer
        = new MapSizeDeserializer();
    private final ArraySizeDeserializer arraySizeDeserializer
        = new ArraySizeDeserializer();
    private final StringSizeDeserializer stringSizeDeserializer
        = new StringSizeDeserializer();
    private final Map<MsgPackType, EzyParser<ByteBuffer, Object>> parsers
        = defaultParsers();
    ...
    protected Object deserialize (ByteBuffer buffer, MsgPackType type) {
        ...
        if (parser == null) {
            throw new IllegalStateException("there is no parse for message type: " + type);
        }
        return parser.parse(buffer);
    }

    protected Map<MsgPackType, EzyParser<ByteBuffer, Object>> defaultParsers () {
        Map<MsgPackType, EzyParser<ByteBuffer, Object>> parsers =
